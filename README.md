...
当前版本：`v0.8.2`；正式发布记录见 GitHub Releases。
...
在“设置 → 408 Dashboard 专注上报”中粘贴个人目录链接或密钥。应用读取目录后默认选中“408 · 强化”，也可切换其他科目和事项；确认后开启开关。只有前台学习页有卡片时，应用才使用 `mutsumi_card` 作为固定来源，每 20 秒发送一次专注心跳。离开学习页、关闭开关或应用进入后台时立即发送 idle。409 会暂停心跳并显示冲突或当日结算提示。

密钥由使用者在设备上输入，使用 Android Keystore 加密保存在不参与系统备份的私有目录，不编入公开安装包，也不进入本地 ZIP/WebDAV 备份。自动升级安装后只需配置一次；换设备需重新填写密钥。服务器以接收时间计算，断联 45 秒后自动结束本次专注。
