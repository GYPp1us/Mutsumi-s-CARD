...
- 当前开发目标是接入 408 Dashboard 专注上报并发布 `v0.8.2`。
- 当前开发分支：`codex/v0.8.2-focus-reporter`。
- 当前隔离工作区：`.worktrees/v0.8.2-focus-reporter`。
...
1. `AGENTS.md`。
2. `docs/superpowers/specs/2026-07-15-v0.5.0双面卡片与Markdown绘图设计.md`，这是双面录入、图层与保存语义的唯一事实来源。
3. `docs/superpowers/specs/2026-07-11-前端响应式布局设计.md`，这是其余前端布局的事实来源。
...
- key 仅文字，value 仅图片。
- 正面图片为可选 value，背面图片为必需 value；正面为空时学习页回退显示 key。保存后的正反面均是 PNG 成品，不保存可编辑笔迹或 Markdown 工程。
- 录入页面仅横屏，左侧共享工具栏，右侧正反面并行银行卡比例画布。Markdown 位于底图之上、笔迹之下，固定在卡面坐标，不随无限世界相机移动。
...
- 推送当前版本标签后等待 GitHub Actions 成功，检查 Release APK 和 SHA-256 附件。
